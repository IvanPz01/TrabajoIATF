...
buttonPredict.addEventListener('click', async () => {
    ...
    const resut = output.dataSync()[0];
    if (resut % 1 >= .5){
        const resuFinal = Math.floor(resut);
        outputText.innerText = `El resultado de la predicción para ${inputField.value} es: ${resuFinal}`;
        return resuFinal;
    }else{
        const resuFinal = resut;
        outputText.innerText = `El resultado de la predicción para ${inputField.value} es: ${resuFinal}`;
        return resuFinal;
    }
})